...
bool Telegram::potentiallyDecrypt(vector<uchar>::iterator& pos)
{
    if (tpl_sec_mode == TPLSecurityMode::AES_CBC_IV)
    {
        ...
        //KPL (Sanxing/Tauron) uses 60 9B instead of standard 2F 2F
        if ((dll_mfct & 0x7fff) == 0x2E0C && a == 0x60 && b == 0x9B)
        {
            *(pos + 0) = 0x2F;
            *(pos + 1) = 0x2F;
            a = 0x2F;
...
//        if ((a != 0x2f || b != 0x2f) && !FUZZING)
        if ((a != 0x2f || b != 0x2f) && !(a == 0x60 && b == 0x9B && (dll_mfct & 0x7fff) == 0x2E0C) && !FUZZING)
        {
            // Wrong key supplied.
